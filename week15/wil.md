...
애플리케이션 (LSM Trees) - 로그 구조
        ↓
파일시스템 (로깅 기법) - 로그 구조
        ↓
SSD (FTL) - 로그 구조
```
각 계층은 각각 쓰기 증폭을 감소하려고 하며 자신의 관점에서 성능 향상을 추구함 

### 로그 스태킹의 문제점 2가지

1. 계층간 소통이 안 됨
- 상위 계층: "이 데이터 곧 삭제해야지"
- 하위 계층: 상위 계층이 뭘 하려는지 모름, "이 데이터 최적화 해야지"   
-> 삭제될 데이터를 쓸데없이 정리하는 낭비 발생

2. 세그먼트 크기가 제각각
**Misaligned writes and discarding of a higher-level log segment**   
<img src='./FileSystem-1.png' width=80% alt='Misaligned writes and discarding of a higher-level log segment'>

- 상위: [    2MB 세그먼트    ]
- 하위: [1MB][1MB][1MB] 세그먼트   
-> 상위 1개가 하위 3개에 걸쳐 있어 관리가 복잡하고 부분 변경시 여러 곳에 영향을 주게 됨

### 다중 쓰기 스트림의 문제점

<img src='./FileSystem-2.png' width=80% alt='Unaligned multistream writes'>

1. 논리적 순차성과 물리적 순차성이 다름   

    ```
    애플리케이션 관점 (논리적):
    스트림 A: [데이터A1] → [데이터A2] → [데이터A3] (순차적)
    스트림 B: [로그B1] → [로그B2] → [로그B3] (순차적)
...
2. 크기 불일치 -> 단편화 발생   
하드웨어 페이지는 고정되어있으나 애플리케이션 데이터의 크기는 가변 크기이므로 단편화가 발생함
    - 페이지 내 빈 공간 낭비
    - 연관된 데이터가 물리적으로 분산됨

### 다중 스트림 문제의 해결책
로그 구조화 저장소의 핵심 목적은 단편화 방지. 하지만 다중 쓰기 스트림 문제로 인해 단편화가 발생함   
이를 해결하려면?   
1. 물리적 분리   
- 데이터 파일과 로그 파일을 저장하는 디스크를 분리해 두 쓰기가 서로 간섭하지 않도록 함
    -> 두 디스크가 동시에 작업하여 전체 처리량이 향상됨   
- 한계
    - 비용: 추가 하드웨어 필요
    - 복잡성: 시스템 관리 복잡도 증가
    - 무엇보다 각 디스크 내에서는 여전히 다중 스트림 문제가 존재하므로 `근본적인 해결책이 아니다!`
2. 하드웨어 정렬   
    ...
    올바른 정렬:
    하드웨어 블록: [    4KB    ][    4KB    ]
    파티션 시작:   ↑ 정확히 4KB 경계에서 시작
    ```
    b. Page Size Alignment
    ```
    크기 불일치:
    쓰기 크기: [3KB] [5KB] [2KB] (가변)
    페이지 크기: [4KB] [4KB] [4KB] (고정)

    크기 정렬:
    쓰기 크기: [4KB] [4KB] [4KB] (페이지 크기와 일치)
    페이지 크기: [4KB] [4KB] [4KB]
    ```
    단편화 자체를 방지하는 근본적인 해결책. 소프트웨어 설계만으로 해결할 수 있으므로 비용면에서도 효율적임

# LLAMA and Mindful Stacking
LLAMA = Latch-free, Log-structured, Access-method Aware

### 기존 로그 스태킹 vs LLAMA
기존에는 각 계층이 독립적으로 최적화해 비효율적인 중복 작업이 발생함   

LLAMA의 경우 Bw-Tree라는 특별한 B-Tree가 LLAMA 저장 시스템 위에 구축됨
```
Bw-Tree: 데이터 구조 관리
LLAMA: Bw-Tree를 이해하고 협력
하드웨어: 더 효율적인 작업 가능
```
LLAMA의 경우 하위 계층에서 상위 계층에서 무엇을 할지 이해하고 협력을 통해 전체적으로 최적화함

## Open-Channel SSDs
기존의 로그 스태킹의 문제를 해결하기 위한 방법으로, 아예 중간 단계를 생략하는 방법
```
기존 (3층):
애플리케이션 → 파일시스템 → SSD FTL → 플래시 메모리

Open-Channel (1층):
애플리케이션 → 플래시 메모리 (직접 접근한다!)
```
### Open-Channel SSD의 특징: FTL 제거
기존에는 SSD가 알아서 가비지 컬렉션과 웨어 레벨링을 처리했던 것과는 달리 Open-Channel은 애플리케이션이 직접 관리함

**장점**:   
- 중복 제거: 로그 스태킹 문제를 차단
- 성능 향상: 불필요한 중간 단계 제거
- 정확한 제어: 하드웨어의 특성을 완전히 활용함

**단점**:   
- 복잡성 증가: 개발자가 하드웨어의 세부사항을 이해해야 함
- 호환성: 기존 시스템과 통합이 어려움
- 개발 비용 증가: 더 많은 전문 지식이 필요함
